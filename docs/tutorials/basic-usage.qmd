...
::: {.callout-note}
## Under the Hood: YOCO + Sparse Matrices

leanfe is fast because it combines two optimizations:

1. **YOCO Compression**: For IID/HC1 SEs, rows with identical (regressors, FEs) are grouped together. With binary treatment × 500 firms × 10 years, 100K rows compress to ~10K unique combinations.

2. **Sparse Matrices**: FE dummies are 99%+ zeros. Sparse storage skips zeros entirely, making matrix operations 10-20x faster.

**Result**: 7-25x speedup with identical coefficients and standard errors.
:::
...
# IID
result_iid = leanfe(data=df, formula="y ~ treatment + x1 + x2 | firm_id + year", vcov="iid")

# HC1 (robust)
result_hc1 = leanfe(data=df, formula="y ~ treatment + x1 + x2 | firm_id + year", vcov="HC1")

# Clustered
result_cluster = leanfe(
    data=df, formula="y ~ treatment + x1 + x2 | firm_id + year",
    vcov="cluster", cluster_cols=["firm_id"]
)
...
start = time.time()
result_polars = leanfe(data=df, formula="y ~ treatment | firm_id + year", backend="polars")
polars_time = time.time() - start

start = time.time()
result_duckdb = leanfe(data=df, formula="y ~ treatment | firm_id + year", backend="duckdb")
