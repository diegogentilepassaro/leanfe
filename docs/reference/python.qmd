...
| `n_compressed` | `int` | Number of compressed rows (if compression used) |
| `compression_ratio` | `float` | Compression ratio (if compression used) |
| `iterations` | `int` | Number of demeaning iterations (0 if compression used) |
...
For `vcov="iid"` or `vcov="HC1"` (without IV), leanfe automatically uses **YOCO compression** from [Wong et al. (2021)](https://arxiv.org/abs/2102.11297):

1. Groups data by (regressors + fixed effects)
2. Computes sufficient statistics per group
3. Runs weighted least squares on compressed data

This is **lossless** - coefficients and standard errors are identical to the full computation, but much faster for discrete regressors (10-1000x compression).
...
├── compress.py          # YOCO compression strategy
├── common.py            # Shared utilities (formula parsing, SE computation)
└── result.py            # LeanFEResult class
```
