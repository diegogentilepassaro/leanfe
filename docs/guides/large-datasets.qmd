...
1. **YOCO Compression**: Automatically compresses data by grouping on (regressors + FEs), achieving 10-1000x compression for discrete regressors
...
For IID and HC1 standard errors, leanfe automatically compresses your data:
...
|---------|--------------|-------|----------|
| **Polars** (default) | Higher | ⚡ Fastest | Data fits in RAM |
| **DuckDB** | 💾 Minimal | Slower | Data larger than RAM |

### Decision Flowchart

```
Does your data fit comfortably in RAM?
├── Yes → Use Polars (default)
└── No → Use DuckDB
    └── Is data in parquet format?
        ├── Yes → Read directly from file path
        └── No → Convert to parquet first
```
...
Parquet is columnar and compressed—ideal for analytical queries:
