...
leanfe uses **YOCO compression** + **sparse matrices** for IID/HC1 standard errors:

1. **Compression**: Groups rows with identical regressors → 10-1000x fewer rows to process
2. **Sparse matrices**: FE dummies are 99%+ zeros → only store and compute non-zeros

The result: **7-25x faster** than traditional methods, with identical coefficients and standard errors.
:::
...
|---------|----------|-----------|
| **Polars** (default) | Speed | Requires data to fit in memory |
| **DuckDB** | Large datasets | Slightly slower, but minimal memory |

Both backends use YOCO compression automatically for IID/HC1 standard errors.
