...
leanfe achieves its speed and memory efficiency through three key innovations:
...
```
Original: 5M rows with binary treatment × 500 firms × 100 products
Compressed: ~100K unique combinations (98% reduction!)
```

The math is **lossless** — coefficients and standard errors are identical to the full computation, just computed on sufficient statistics (count, sum, sum of squares) per group.
...
2. **Sparse cluster matrix W̃_C**: Maps groups to clusters (G × C matrix with one 1 per row)
3. **Vectorized aggregation**: `cluster_scores = W̃_Cᵀ @ (X × ẽ⁰)` — no loops over clusters!
...
[Learn more about the methodology →](benchmarks/methodology.qmd)
...
**YOCO + sparse matrices** make leanfe extremely fast for **all standard error types** — IID, HC1, and clustered. DuckDB uses far less memory and can process datasets larger than RAM.
...
[View IID/HC1 benchmarks →](benchmarks/overview.qmd) | [Clustered SE benchmarks →](benchmarks/cluster-se.qmd)
...
[Browse tutorials →](tutorials/basic-usage.qmd)
:::
...
[View benchmarks →](benchmarks/overview.qmd)
:::
