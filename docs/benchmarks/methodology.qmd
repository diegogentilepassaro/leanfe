...
leanfe uses the **YOCO (You Only Compress Once)** strategy from [Wong et al. (2021)](https://arxiv.org/abs/2102.11297) for IID and HC1 standard errors:

1. **Compress**: GROUP BY (regressors + fixed effects) → compute `n`, `sum(y)`, `sum(y²)` per group
2. **Solve**: Run weighted least squares on compressed data
3. **Standard errors**: Compute from grouped residual sum of squares

This is **lossless** - coefficients and standard errors are identical to the full computation, but much faster for discrete regressors.
