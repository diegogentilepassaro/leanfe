...
For IID and HC1 standard errors, leanfe automatically:

1. **Compresses data**: GROUP BY (regressors + fixed effects) → compute `n`, `sum(y)`, `sum(y²)` per group
...
4. **Computes standard errors**: From grouped residual sum of squares
...
|--------|---------------|---------------|
| **Speed** | ⚡ Fastest (0.08s @ 1M rows) | Fast (0.5s @ 1M rows) |
