...
1. **Compresses data**: GROUP BY (regressors + fixed effects + cluster) → compute `n`, `sum(y)`, `sum(y²)` per group
...
For clustered SEs, leanfe uses a sparse cluster indicator matrix W̃_C:

- **W̃_C ∈ R^{G×C}**: Maps G compressed groups to C clusters (one 1 per row)
- **Vectorized aggregation**: `cluster_scores = W̃_Cᵀ @ (X × ẽ⁰)` — no loops over clusters!
- **Meat matrix**: `meat = cluster_scoresᵀ @ cluster_scores`
