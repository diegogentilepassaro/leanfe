...
leanfe is designed for speed and memory efficiency. It uses **YOCO (You Only Compress Once)** compression from [Wong et al. (2021)](https://arxiv.org/abs/2102.11297) to dramatically reduce computation time and memory usage.

## How Compression Works

For IID and HC1 standard errors, leanfe automatically compresses data by grouping on (regressors + fixed effects):

1. **GROUP BY** regressors and fixed effects
2. **Compute sufficient statistics**: count, sum(y), sum(y²) per group
3. **Run weighted least squares** on compressed data
4. **Compute standard errors** from grouped residuals

This is lossless - coefficients and standard errors are identical to the full computation.
