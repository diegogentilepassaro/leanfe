...
You don't need to configure anything — leanfe picks the optimal strategy automatically.

### YOCO Compression (Low-Cardinality FEs)

From [Wong et al. (2021)](https://arxiv.org/abs/2102.11297):

1. **Compress data**: GROUP BY (regressors + FEs) → compute sufficient statistics per group
...
|---------|-------|--------|----------|
| **leanfe-Polars** | ⚡ Fastest | Moderate | Maximum speed, in-memory data |
| **leanfe-DuckDB** | Fast | 💾 Minimal | Large datasets, memory-constrained |
...
# pyfixest is optional - may not be installed in CI
try:
...
def run_benchmark(n_obs, include_pyfixest=True):
    ...
    # pyfixest benchmark (skip for very large datasets - too slow, or if not installed)
    if include_pyfixest and HAS_PYFIXEST and n_obs <= 1_000_000:
        gc.collect()
        df_pd = df.to_pandas()
        tracemalloc.start()
        start = time.time()
...
for n in sizes:
    result = run_benchmark(n, include_pyfixest=HAS_PYFIXEST and (n <= 1_000_000))
...
pyfixest_note = "pyfixest skipped for 10M rows (too slow)" if HAS_PYFIXEST else "pyfixest not installed"
print(f"\nNote: {pyfixest_note}")
```
